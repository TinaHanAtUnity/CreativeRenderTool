...
export class VastCompanionAdStaticResourceValidator implements IValidator {
    ...
    private validateStaticResourceUrl(companionAd: VastCompanionAdStaticResource) {
        ...
        if (staticResourceURL === null) {
            this._errors.push(new CampaignError(`VAST Companion ad(${adId}) is missing required StaticResource Element`, ProgrammaticVastParser.ContentType, CampaignErrorLevel.MEDIUM, VastErrorCode.COMPANION_RESOURCE_NOT_FOUND, undefined, undefined));
        } else if (!Url.isValidProtocol(staticResourceURL)) {
            this._errors.push(new CampaignError(VastValidationUtilities.invalidUrlError(`companion ad(${adId}) staticResourceUrl`, staticResourceURL).message, ProgrammaticVastParser.ContentType, CampaignErrorLevel.MEDIUM, VastErrorCode.COMPANION_RESOURCE_NOT_FOUND, undefined, staticResourceURL));
        }
    }
    ...
    private validateCreativeType(companionAd: VastCompanionAdStaticResource) {
        ...
        if (creativeType === null) {
            this._errors.push(new CampaignError(`VAST Companion ad(${adId}) "StaticResource" is missing required "creativeType" attribute`, ProgrammaticVastParser.ContentType, CampaignErrorLevel.MEDIUM, VastErrorCode.COMPANION_RESOURCE_NOT_FOUND, undefined, staticResourceURL));
        } else if (VastCompanionAdStaticResourceValidator._supportedCreativeTypes.indexOf(creativeType.toLowerCase()) === -1) {
            this._errors.push(new CampaignError(`VAST Companion ad(${adId}) "StaticResource" attribute "creativeType=${creativeType}" is not supported`, ProgrammaticVastParser.ContentType, CampaignErrorLevel.MEDIUM, VastErrorCode.COMPANION_RESOURCE_NOT_FOUND, undefined, staticResourceURL));
        }
    }
    ...
    private validateCreativeDimensions(companionAd: VastCompanionAdStaticResource) {
        ...
        if (height > width) {   // Portrait
            if (height < VastCompanionAdStaticResourceValidator._minPortraitHeight || width < VastCompanionAdStaticResourceValidator._minPortraitWidth) {
                this._errors.push(new CampaignError(`VAST Companion ad(${adId}) "StaticResource" is not meeting minimum size 320 x 480`, ProgrammaticVastParser.ContentType, CampaignErrorLevel.MEDIUM, VastErrorCode.COMPANION_SIZE_UNSUPPORTED, undefined, staticResourceURL));
            }
        } else {
            if (height < VastCompanionAdStaticResourceValidator._minLandscapeHeight || width < VastCompanionAdStaticResourceValidator._minLandscapeWidth) {
                this._errors.push(new CampaignError(`VAST Companion ad(${adId}) "StaticResource" is not meeting minimum size 480 x 320`, ProgrammaticVastParser.ContentType, CampaignErrorLevel.MEDIUM, VastErrorCode.COMPANION_SIZE_UNSUPPORTED, undefined, staticResourceURL));
            }
        }
    }
    ...
    private validateCompanionClickThroughURLTemplate(companionAd: VastCompanionAdStaticResource) {
        ...
        if (companionClickThroughURLTemplate === null) {
            ...
            this._errors.push(new CampaignError(`VAST Companion ad(${adId}) is missing required CompanionClickThrough Element`, ProgrammaticVastParser.ContentType, CampaignErrorLevel.LOW, VastErrorCode.COMPANION_NO_CLICKTHROUGH, undefined, staticResourceURL));
        } else if (!Url.isValidProtocol(companionClickThroughURLTemplate)) {
            this._errors.push(new CampaignError(VastValidationUtilities.invalidUrlError(`companion ad(${adId}) companionClickThroughURLTemplate`, companionClickThroughURLTemplate).message, ProgrammaticVastParser.ContentType, CampaignErrorLevel.LOW, VastErrorCode.COMPANION_NO_CLICKTHROUGH, undefined, staticResourceURL));
        }
    }
        ...
        Object.keys(trackingEvents).map((key) => {
            ...
            urls.map((url) => {
                if (!Url.isValidProtocol(url)) {
                    // Error level LOW
                    this._errors.push(new CampaignError(VastValidationUtilities.invalidUrlError('companion ad trackingEvents', url).message, ProgrammaticVastParser.ContentType, CampaignErrorLevel.LOW, VastErrorCode.INVALID_URL_ERROR, undefined, url));
                }
            });
        });
    }
