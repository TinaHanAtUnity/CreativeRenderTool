...
export class VastLinearCreativeValidator implements IValidator {
    ...
    private validateDuration(linearCreative: VastCreativeLinear) {
        if (linearCreative.getDuration() === -1) {
            // Error level HIGH
            this._errors.push(new CampaignError(VastErrorInfo.errorMap[VastErrorCode.DURATION_UNSUPPORTED], ProgrammaticVastParser.ContentType, CampaignErrorLevel.HIGH, VastErrorCode.DURATION_UNSUPPORTED));
        }
    }
    ...
    private validateVideoClickThroughURLTemplate(linearCreative: VastCreativeLinear) {
        ...
        if (videoClickThroughURLTemplate && !Url.isValidProtocol(videoClickThroughURLTemplate)) {
            // Error level HIGH
            this._errors.push(new CampaignError(VastValidationUtilities.invalidUrlError('linear creative videoClickThroughURLTemplate', videoClickThroughURLTemplate).message, ProgrammaticVastParser.ContentType, CampaignErrorLevel.HIGH, VastErrorCode.MEDIA_FILE_NO_CLICKTHROUGH_URL, undefined, videoClickThroughURLTemplate));
        }
    }
    ...
    private validateVideoClickTrackingURLTemplates(linearCreative: VastCreativeLinear) {
        ...
        videoClickTrackingURLTemplates.map((url) => {
            if (!Url.isValidProtocol(url)) {
                // Error level LOW
                this._errors.push(new CampaignError(VastValidationUtilities.invalidUrlError('linear creative videoClickTrackingURLTemplates', url).message, ProgrammaticVastParser.ContentType, CampaignErrorLevel.LOW, VastErrorCode.INVALID_URL_ERROR, undefined, url));
            }
        });
    }
    ...
    private validateVideoCustomClickURLTemplates(linearCreative: VastCreativeLinear) {
        ...
        videoCustomClickURLTemplates.map((url) => {
            if (!Url.isValidProtocol(url)) {
                // Error level LOW
                this._errors.push(new CampaignError(VastValidationUtilities.invalidUrlError('linear creative videoCustomClickURLTemplates', url).message, ProgrammaticVastParser.ContentType, CampaignErrorLevel.LOW, VastErrorCode.INVALID_URL_ERROR, undefined, url));
            }
        });
    }
}
