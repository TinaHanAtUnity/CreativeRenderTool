...
describe('EndScreen', () => {
    ...
    xit('should render', () => {
        const endScreen = new PerformanceEndScreen(nativeBridge, TestFixtures.getCampaign(), true, 'en', 'testGameId', TestFixtures.getBuildInformation());
        endScreen.render();
        assert.equal(endScreen.container().innerHTML, EndScreenFixture);
    });

    it('should render with translations', () => {
        const endScreen = new PerformanceEndScreen(nativeBridge, TestFixtures.getCampaign(), true, 'fi', 'testGameId', TestFixtures.getBuildInformation());
        endScreen.render();
