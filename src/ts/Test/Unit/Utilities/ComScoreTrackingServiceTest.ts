...
describe('ComScoreTrackingServiceTest', () => {
    ...
    describe('when calling sendEvent', () => {
        it('thirdPartyManger\'s sendEvent is called', () => {
            comscoreService.sendEvent('play', TestFixtures.getSession().getId(), '20', 15, TestFixtures.getCampaign().getCreativeId(), TestFixtures.getCampaign().getCategory());
    ...
    describe('when constructing url via setEventUrl()', () => {
        ...
        const fillComscoreParams = () => {
            comscoreService.sendEvent('play', TestFixtures.getSession().getId(), '20', 0, TestFixtures.getCampaign().getCreativeId(), TestFixtures.getCampaign().getCategory());
            urlPlay = spy.args[0][2];
            comscoreService.sendEvent('end', TestFixtures.getSession().getId(), '20', 15, TestFixtures.getCampaign().getCreativeId(), TestFixtures.getCampaign().getCategory());
            urlEnd = spy.args[1][2];
