...
export class OMIDEventBridge {
    ...
    constructor(core: ICoreApi, handler: IOMIDEventHandler, iframe: HTMLIFrameElement, openMeasurement: OpenMeasurement, campaign: Campaign) {
        this._core = core;
    ...
    public postVideoAdEventMessage(event: IVerificationEvent, uuid: string) {
        ...
        if (event.type === 'omidImpression' && this._campaign instanceof AdMobCampaign) {
            ProgrammaticTrackingService.reportMetricEvent(AdmobMetric.AdmobOMRegisteredImpression);
        }
