...
import { LoadExperiment, LoadExperimentWithCometRefreshing, LoadExperimentWithCometRefreshingAfterAnyStart, ZyngaLoadExperiment, ZyngaLoadExperimentWithCometRefreshing, LoadRefreshV3, LoadRefreshV4, LoadExperiment16Exclusive } from 'Core/Models/ABGroup';
...
export class Ads implements IAds {
    ...
    private configureRefreshManager(): void {
        if (this._loadApiEnabled) {
            ...
            // AbGroup 5, 6 - V4 Refresh Load
            if (LoadRefreshV4.isValid(abGroup)) {
                if (abGroup === 5) {
                    // AbGroup 5 - V4 Refresh Load
                ...
                } else if (abGroup === 6 && !isZyngaDealGame) {
                    // ABGroup 6 and Not Zynga game - V4 Refresh Load
                    this.RefreshManager = new PerPlacementLoadManagerV4(this.Api, this.Config, this._core.Config, this.CampaignManager, this._core.ClientInfo, this._core.FocusManager, this._core.ProgrammaticTrackingService);
                } else if (abGroup === 6 && isZyngaDealGame) {
                    // ABGroup 6 - Original Load
            ...
            } else if (isZyngaDealGame) {
                // if Zynga game not on Abgroup 5, 12 - Original Load
                this.RefreshManager = new PerPlacementLoadManager(this.Api, this.Config, this._core.Config, this.CampaignManager, this._core.ClientInfo, this._core.FocusManager, this._core.ProgrammaticTrackingService);
            } else if (!isZyngaDealGame && LoadExperiment16Exclusive.isValid(abGroup)) {
                // if Zynga game not on Abgroup 5, 6, 12 - use Original Load
                this.RefreshManager = new PerPlacementLoadManager(this.Api, this.Config, this._core.Config, this.CampaignManager, this._core.ClientInfo, this._core.FocusManager, this._core.ProgrammaticTrackingService);
            }
        } else {
