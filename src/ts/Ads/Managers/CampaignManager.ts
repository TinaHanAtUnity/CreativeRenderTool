...
export class CampaignManager {
        ...
        if('placements' in json) {
            ...
            for(const mediaId in fill) {
                if(fill.hasOwnProperty(mediaId)) {
                    ...
                    if(contentType && contentType !== CometCampaignParser.ContentType && typeof cacheTTL !== 'undefined' && cacheTTL > 0 && (cacheTTL < refreshDelay || refreshDelay === 0)) {
    ...
    private handleCampaign(response: AuctionResponse, session: Session): Promise<void> {
        ...
        return parser.parse(response, session).catch((error) => {
            if (error instanceof CampaignError && error.contentType === ProgrammaticVastParser.ContentType && error.errorCode === ProgrammaticVastParser.MEDIA_FILE_GIVEN_VPAID_IN_VAST_AD) {
                parser = this.getCampaignParser(ProgrammaticVPAIDParser.ContentType);
